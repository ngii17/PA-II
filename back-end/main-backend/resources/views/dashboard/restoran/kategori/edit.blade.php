...
<div class="edit-page-wrapper">
    ...
    <div class="edit-header">
        ...
        <h2>Edit <span>Kategori</span></h2>
        <p><i class="fas fa-tag me-1"></i> Ubah informasi kategori menu restoran</p>
    </div>
        ...
        <div class="col-lg-8">
            <div class="card-premium">
                <div class="card-premium-header">
                    <h6><i class="fas fa-pen-alt"></i> Form Ubah Kategori</h6>
                </div>
                <div class="card-premium-body">

                    {{-- Error Alerts (dari validasi Laravel) --}}
                    @if ($errors->any())
                        <div class="alert-error-premium" style="background: linear-gradient(135deg, #fff5f5 0%, #fee2e2 100%); border-left: 4px solid var(--rose); border-radius: 16px; padding: 16px 20px; margin-bottom: 24px;">
                            <ul style="margin: 0; padding-left: 20px; color: #991b1b; font-weight: 500; font-size: .8rem;">
                    ...
                    <div class="info-box-premium">
                        ...
                        <div class="info-text">
                            Perubahan nama kategori akan langsung mempengaruhi tampilan menu di halaman pemesanan restoran.
                        </div>
                    </div>

                    <form action="{{ route('dashboard.restoran.kategori.update', $kategori->id) }}" method="POST" id="editKategoriForm">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" 
                                      ...
                                      rows="4"
                                      placeholder="Tambahkan penjelasan tentang kategori ini...">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
        ...
        <div class="col-lg-4">
            ...
            <div class="summary-card">
                ...
                <div class="summary-row">
                    <span class="summary-label">ID Kategori</span>
                    <span class="summary-value">#{{ $kategori->id }}</span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Nama Kategori</span>
                    <span class="summary-value">{{ $kategori->nama_kategori }}</span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Total Menu</span>
                    <span class="summary-value highlight">{{ $kategori->menus->count() }} menu</span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Dibuat pada</span>
                    <span class="summary-value">{{ $kategori->created_at->format('d M Y, H:i') }}</span>
                </div>
                <div class="summary-row">
                    <span class="summary-label">Terakhir diupdate</span>
                    <span class="summary-value">{{ $kategori->updated_at->diffForHumans() }}</span>
                </div>
            </div>
            ...
            <div class="card-premium mt-4">
                <div class="card-premium-header">
                    <h6><i class="fas fa-lightbulb"></i> Tips & Catatan</h6>
                </div>
                <div class="card-premium-body">
                    <ul class="small text-muted mb-0" style="padding-left: 18px;">
                        <li class="mb-2">Nama kategori yang jelas memudahkan pelanggan mencari menu favorit.</li>
                        <li class="mb-2">Gunakan nama yang singkat, deskriptif, dan mudah diingat.</li>
                        <li>Jika kategori memiliki banyak menu, pertimbangkan untuk membagi menjadi sub-kategori.</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card-premium mt-4">
                <div class="card-premium-header">
                    <h6><i class="fas fa-eye"></i> Preview Nama Kategori</h6>
                </div>
                <div class="card-premium-body">
                    <div class="text-center p-3 rounded-3" style="background: var(--surface-2);">
                        <div class="fw-bold text-primary" id="previewNamaKategori">
                            {{ $kategori->nama_kategori }}
                        </div>
                        <small class="text-muted">Akan tampil seperti ini di menu restoran</small>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Animasi Cards
    const cards = document.querySelectorAll('.card-premium, .summary-card');
    ...
    if (namaInput && previewElement) {
        namaInput.addEventListener('input', function() {
            ...
            previewElement.textContent = value !== '' ? value : '(Nama kategori akan muncul di sini)';
        });
    }
});

// Form validation sebelum submit (sama seperti kode asli)
