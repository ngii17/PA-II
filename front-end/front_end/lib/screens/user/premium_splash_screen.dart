...
import '../home/home_screen.dart'; // Navigasi Langsung ke Home
import '../../providers/event_provider.dart';
import '../../widgets/splash_widgets.dart'; // Pastikan SharpStarPainter ada di sini

// ══════════════════════════════════════════════════════════════════════════════
//  TIMELINE PEMBUKA PURNAMA (Total 8.5 detik)
// ══════════════════════════════════════════════════════════════════════════════
...
class _PremiumSplashScreenState extends State<PremiumSplashScreen>
  ...
  void initState() {
    ...
    // 1. Ambil Tema Aktif dari Port 8001 saat animasi berjalan (Background Sync)
    ...
    // 2. Inisialisasi Timeline Utama
    _main = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 8500),
    )..forward();

    // 3. Loop Partikel Emas
    _particle = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 2800),
    )..repeat();

    // 4. Handler Navigasi Selesai
    _main.addStatusListener((status) {
      if (status == AnimationStatus.completed && mounted) {
        Navigator.pushReplacement(
          context,
          PageRouteBuilder(
            pageBuilder: (_, __, ___) => const HomeScreen(), // LANGSUNG KE HOME
  ...
  // Fungsi Helper Kalkulasi Progress Animation
        ...
        builder: (ctx, _) {
          ...
          return Stack(
            ...
            children: [
              ...
              // --- 4. LIGHT BURST (MEKAR CAHAYA TENGAH) ---
              if (v >= 0.18 && v <= 0.52) _buildLightBurst(v, size),

              // --- 5. FLASH KUNING (SEKETIKA) ---
              if (v >= 0.20 && v <= 0.40) _buildFlash(v),

              // --- 6. PARTIKEL DEBU EMAS ---
              if (v >= 0.30)
                AnimatedBuilder(
                  animation: _particle,
  ...
  // ──────────────────────────────────────────────────────────────────────────
  //  UI COMPONENTS (TETAP SESUAI CODE ORIGINAL ANDA)
  // ──────────────────────────────────────────────────────────────────────────
    ...
    return Center(
      child: Opacity(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [const Color(0xFFFFE57F).withOpacity(0.95), const Color(0xFFD4AF37).withOpacity(0.45), Colors.transparent])),
        ),
      ),
    );
  }
  ...
  Widget _buildFlash(double v) {
    ...
    return Opacity(opacity: (opacity * 0.72).clamp(0.0, 1.0), child: Container(color: const Color(0xFFD4AF37)));
  }
  ...
  Widget _buildTopOrnament(double v) {
    ...
    return Opacity(
      ...
      child: Transform.translate(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [_diamond(3.5), const SizedBox(width: 7), _line(56 * p), const SizedBox(width: 7), _diamond(6), const SizedBox(width: 7), _line(56 * p), const SizedBox(width: 7), _diamond(3.5)]),
      ),
    );
  }
        ...
        return Transform.translate(
          ...
          child: Opacity(
            ...
            child: Padding(
              ...
              child: ShaderMask(
                shaderCallback: (b) => const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xFFFFE57F), Color(0xFFD4AF37), Color(0xFF9A7B1A)], stops: [0.0, 0.45, 1.0]).createShader(b),
                child: Text(text[i], style: const TextStyle(color: Colors.white, fontSize: 58, fontWeight: FontWeight.w900, letterSpacing: 2, height: 1.0)),
              ),
            ),
          ),
        );
  ...
  Widget _buildDivider(double v) {
    ...
    return Opacity(opacity: p, child: Container(width: 150 * p, height: 1, decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.transparent, const Color(0xFFD4AF37).withOpacity(0.85), Colors.transparent]))));
  }

  Widget _buildSubtitle(double v) {
    final double p = Curves.easeOut.transform(_p(v, 0.78, 0.94));
    return Opacity(opacity: p, child: Transform.translate(offset: Offset(0, 8 * (1 - p)), child: const Text('Hotel & Restaurant Balige', style: TextStyle(color: Color(0xFF8A96B0), fontSize: 13, fontStyle: FontStyle.italic, letterSpacing: 2.8, fontWeight: FontWeight.w300))));
  }

  Widget _buildBottomOrnament(double v) {
    final double p = Curves.easeOut.transform(_p(v, 0.84, 0.97));
    return Opacity(opacity: p, child: Transform.translate(offset: Offset(0, 8 * (1 - p)), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [_line(38 * p), const SizedBox(width: 8), _diamond(3), const SizedBox(width: 8), _line(38 * p)])));
  }

  Widget _diamond(double s) => Transform.rotate(angle: pi / 4, child: Container(width: s, height: s, color: const Color(0xFFD4AF37)));
  Widget _line(double w) => Container(width: w.clamp(0.0, double.infinity), height: 1, color: const Color(0xFFD4AF37).withOpacity(0.5));
}
...
class _ShockwavePainter extends CustomPainter {
  final double progress; final double maxRadius;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      canvas.drawCircle(center, radius, Paint()..color = const Color(0xFFD4AF37).withOpacity(opacity.clamp(0.0, 1.0))..style = PaintingStyle.stroke..strokeWidth = (3.5 - i * 0.9) * (1 - ep * 0.5));
    }
  }
  @override bool shouldRepaint(covariant _ShockwavePainter o) => o.progress != progress;
}

class _GoldParticlePainter extends CustomPainter {
  final double loopT; final double globalOpacity;
  static final _rng = Random(99);
  static final List<_Pt> _pts = List.generate(32, (i) => _Pt(x: _rng.nextDouble(), y: _rng.nextDouble(), speed: 0.03 + _rng.nextDouble() * 0.05, r: 1.2 + _rng.nextDouble() * 2.8, phase: _rng.nextDouble()));
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in _pts) {
      ...
      paint..color = const Color(0xFFD4AF37).withOpacity(globalOpacity * 0.10)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2.5);
      canvas.drawCircle(Offset(dx, dy), p.r * 2, paint);
      paint..color = const Color(0xFFFFE57F).withOpacity(globalOpacity * 0.65)..maskFilter = null;
      canvas.drawCircle(Offset(dx, dy), p.r * 0.5, paint);
    }
  }
  @override bool shouldRepaint(covariant _GoldParticlePainter o) => o.loopT != loopT || o.globalOpacity != globalOpacity;
}
